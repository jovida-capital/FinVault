...
def fetch_yahoo(ticker):
    ...
    try:
        ...
        # ── Contrôle de cohérence entre le quote et la série de clôtures ──
        # Pour certains instruments (WSRI.PA notamment), meta.regularMarketPrice
        # provient d'une cotation différente de celle du graphique : devise ou
        # place de cotation distincte. Injecter ce cours dans l'historique y
        # introduirait un point aberrant, qui deviendrait ensuite J-1 puis J-2
        # avant d'être corrigé au passage suivant.
        # Au-delà de 12 % d'écart avec la dernière clôture connue, on considère
        # que le quote n'est pas comparable et on retient la clôture.
        quote_incoherent = False
        if history:
            derniere = history[max(history)]
            if derniere > 0 and abs(price - derniere) / derniere > 0.12:
                print(f"\n    ATTENTION {ticker} : quote {price} incohérent avec la "
                      f"dernière clôture {derniere} ({(price-derniere)/derniere*100:+.1f} %) "
                      f"— clôture retenue", end='')
                price = derniere
                quote_incoherent = True
...
def main():
    ...
    for i, (ticker, nom) in enumerate(tickers.items()):
        ...
        if result:
            ...
            # Le cours instantané ne complète l'historique que s'il apporte une
            # information nouvelle. Avant l'ouverture — le script tourne à 5h —
            # regularMarketPrice vaut encore la clôture de la veille : l'injecter
            # sous la date du jour créerait une séance fantôme, qui deviendrait
            # J-1 le lendemain avant d'être corrigée.
            derniere_cloture = result['history'][max(result['history'])] if result['history'] else None
            apporte_du_neuf = (
                derniere_cloture is None
                or abs(result['price'] - derniere_cloture) > 0.0001 * max(1.0, derniere_cloture)
            )
            if (today_str not in result['history']
                    and not result.get('quote_incoherent')
                    and apporte_du_neuf):
