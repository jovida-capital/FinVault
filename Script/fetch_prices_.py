...
def fetch_yahoo(ticker):
    ...
    try:
        ...
        # ── Coherence entre le cours instantane et la serie de clotures ──
        # Pour certains instruments (WSRI.PA), regularMarketPrice provient d'une
        # cotation differente de celle du graphique. L'injecter dans l'historique
        # y creerait un point aberrant qui deviendrait ensuite J-1 puis J-2.
        quote_incoherent = False
        quote_brut = price
        if history:
            derniere = history[max(history)]
            if derniere > 0 and abs(price - derniere) / derniere > 0.12:
                print(f"\n    ATTENTION {ticker} : quote {price} incoherent avec la "
                      f"derniere cloture {derniere} "
                      f"({(price - derniere) / derniere * 100:+.1f} %) — cloture retenue",
                      end='')
                price = derniere
        ...
        return {
            ...
            'quote_brut': round(quote_brut, 4) if quote_incoherent else None,
            'price': round(price, 4),
            'currency': currency,
            'name': name,
            'history': history,
...
def main():
    ...
    for i, (ticker, nom) in enumerate(tickers.items()):
        ...
        if result:
            ...
            # Nettoyage des valeurs parasites héritées des exécutions antérieures
